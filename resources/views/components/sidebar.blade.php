<div x-data="{ open: false }" class="font-sans antialiased">
    <aside class="hidden lg:flex fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-100 h-screen flex-col justify-between shadow-[2px_0_20px_rgba(0,0,0,0.02)]">
        <div>
            ...
            <nav class="p-4 space-y-1.5">
                @php
                    $navLinks = [
                        ['route' => 'dashboard', 'label' => 'Asosiy panel', 'icon' => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'],
                        ['route' => 'followers', 'label' => 'Obunachilar', 'icon' => 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z'], 
                        ['route' => 'chat', 'label' => 'Xabarlar', 'icon' => 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z'],
                        ['route' => 'notifications', 'label' => 'Bildirishnomalar', 'icon' => 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9'],
                        ['route' => 'profile.show', 'label' => 'Profil', 'icon' => 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z', 'params' => ['username' => auth()->user()->username]],
                    ];
                @endphp
    ...
    <div class="lg:hidden fixed bottom-6 left-6 right-6 z-50">
        <div class="flex items-center justify-between bg-[#0F1419] shadow-[0_20px_40px_rgba(0,0,0,0.3)] rounded-full px-7 py-3.5 border border-white/5 backdrop-blur-lg">
            ...
            <a href="{{ route('dashboard') }}" class="p-2 {{ request()->routeIs('dashboard') ? 'text-white' : 'text-gray-500' }}">
            ...
            <a href="{{ route('chat') }}" class="p-2 {{ request()->routeIs('chat') ? 'text-white' : 'text-gray-500' }}">
            ...
            <a href="{{ route('notifications') }}" class="p-2 relative {{ request()->routeIs('notifications') ? 'text-white' : 'text-gray-500' }}">
            ...
            <a href="{{ route('profile.show', ['username' => auth()->user()->username]) }}" class="p-0.5 rounded-full {{ request()->routeIs('profile*') ? 'ring-2 ring-white' : 'opacity-60' }}">
